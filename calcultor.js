...
export async function menjer_rouer_Product_Data(req, res) {
  try {
...
export async function menjer_rouer_cart_Data(req, res) {
  try {
...
export async function menjerCheckOut(req, res) {
  try {
    ...
    if (!check_balance(client, obj_of_products)) {
      res
        .status(400)
        .json({ message: "There isn't enough money in the account." });
      return;
    }
    ...
    const cliant_to_update = Updating_the_balance_sheet(
      Total_amount_to_be_deducted,
      obj_of_clients,
      customerId,
    );
...
export async function get_Order_History(req, res) {
  try {
    ...
    const customer_order = orders.filter(
      (order) => order.customer === +id_customer,
    );
